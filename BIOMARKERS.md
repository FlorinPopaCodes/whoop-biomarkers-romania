# Biomarkers

What each provider calls each Whoop biomarker, and who sells it. `—` means the
provider sells nothing that yields it; `?` means undetermined. Where a biomarker
is only obtainable inside a panel, the cell names the panel.

Prices are in the shopping lists: [Synevo](SHOPPING-LIST-SYNEVO.md) ·
[Regina Maria](SHOPPING-LIST-REGINA-MARIA.md) ·
[MedLife](SHOPPING-LIST-MEDLIFE.md). Which provider comes out cheapest is in
[README](README.md).

## Core

| Whoop Biomarker | Synevo | Regina Maria | MedLife |
|---|---|---|---|
| Alanine Aminotransferase (ALT) | Alaninaminotransferaza (GPT, ALAT, ALT) | Alaninaminotransferaza (ALT/GPT/TGP) | TGP/ALT |
| Albumin | Albumina serica | Albumina serica | Albumina serica |
| Alkaline Phosphatase (ALP) | Fosfataza alcalina | Fosfataza alcalina | Fosfataza alcalina |
| Apolipoprotein B (ApoB) | Apolipoproteina B | Apolipoproteina B | Apolipoproteina B |
| Aspartate Aminotransferase (AST) | Aspartataminotransferaza (GOT/ASAT/AST) | Aspartataminotransferaza (GOT/AST/TGO) | TGO/AST |
| Basophil % | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Basophils | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Blood Fasting Glucose | Glucoza serica (glicemie) | Glucoza serica | Glucoza serica |
| Blood Urea Nitrogen | Uree serică | Uree serica | Ureea nitrogen (BUN) |
| Calcium | Calciu seric | Calciu seric | Calciu seric total |
| Carbon Dioxide | Bicarbonat (ECO2) | Rezerva alcalina (ECO2) | Bicarbonat seric |
| Chloride | Clor seric | Clor seric | Clor seric |
| Cortisol | Cortizol seric | Cortizol | Cortisol seric |
| Creatinine | Creatinina serica | Creatinina serica | Creatinina serica |
| Eosinophil % | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Eosinophils | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Estradiol | Estradiol | Estradiol | Estradiol |
| Ferritin | Feritina | Feritina | Feritina |
| Follicle Stimulating Hormone (FSH) | FSH (hormon de stimulare foliculară) | FSH (hormon de stimulare foliculara) | FSH |
| Free Testosterone | Testosteron liber | Testosteron liber | Testosteron liber |
| Glucose | Glucoza serica (glicemie) | Glucoza serica | Glucoza serica |
| HDL Cholesterol | Colesterol HDL | HDL Colesterol | HDL- Colesterol |
| Hematocrit | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Hemoglobin | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Hemoglobin A1c (HbA1c) | Hemoglobina glicata (Hb A1c) | Hemoglobina glicozilata/HbA1c | Hemoglobina glicozilata (HbA1C) |
| High-Sensitivity C-Reactive Protein (hs-CRP) | Proteina C reactiva (hsCRP) inalt sensibila | Proteina C reactiva inalt sensibila (HSCRP) | CRP hs |
| Homocysteine | Homocisteina | Homocisteina | Homocisteina |
| Insulin | Insulina | Insulina | Insulina |
| Iron | Sideremie | Fier seric(sideremie) | Sideremie |
| LDL Cholesterol | Colesterol LDL | LDL Colesterol | LDL-Colesterol |
| Lipoprotein (a) | Lipoproteina (a) | Lipoproteina A | Lipoproteina (a) |
| Luteinizing Hormone (LH) | LH (hormon luteinizant) | LH (Hormon luteinizant) | LH |
| Lymphocyte % | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Lymphocytes | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Mean Corpuscular Hemoglobin (MCH) | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Mean Corpuscular Hemoglobin Concentration (MCHC) | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Mean Corpuscular Volume (MCV) | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Mean Platelet Volume (MPV) | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Monocyte % | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Monocytes | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Neutrophil % | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Neutrophils | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Platelets | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Potassium | Potasiu seric | Potasiu seric | Potasiu seric |
| Red Blood Cell Count (RBC) | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Red Cell Distribution Width (RDW) | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |
| Sex Hormone Binding Globulin (SHBG) | SHBG (sex hormone binding globuline) | SHBG (sex hormone binding globulin) | SHBG |
| Sodium | Sodiu seric | Sodiu seric | Sodiu seric |
| Testosterone | Testosteron | Testosteron | Testosteron |
| Thyroid-Stimulating Hormone (TSH) | TSH (hormon de stimulare tiroidiană) | TSH (Hormon de stimulare tiroidiana) | TSH |
| Total Bilirubin | Bilirubina totala | Bilirubina totala | Bilirubina totala |
| Total Cholesterol | Colesterol total | Colesterol total | Colesterol total |
| Total Iron-Binding Capacity (TIBC) | Capacitatea totala de legare a fierului (CTLF) | Capacitatea totala de legare a fierului (CTLF) | CTLFe (Capacitatea totala de legare a fierului) |
| Total Protein | Proteine totale serice | Proteine totale serice | Proteine totale |
| Triglycerides | Trigliceride | Trigliceride | Trigliceride |
| Vitamin D | 25-OH-vitamina D | 25 OH Vitamina D | 25-OH Vitamina D |
| White Blood Cells (WBC) | Hemograma cu formula leucocitara cu Hb, Ht si indici | Hemoleucograma cu formula leucocitara,Hb,Ht, indici eritrocitari | Hemoleucograma completa |

All three providers sell all 57.

**Core — Derived (18)**

| Derived Biomarker | Formula |
|---|---|
| Albumin/Globulin Ratio | = Albumin ÷ Globulin |
| Anion Gap | = Na − (Cl + CO₂) |
| Atherogenic Index of Plasma (AIP) | = log₁₀(Triglycerides ÷ HDL), molar units |
| BUN/Creatinine Ratio | = BUN ÷ Creatinine |
| Cholesterol/HDL Ratio | = Total Cholesterol ÷ HDL |
| Corrected Calcium | = Calcium + 0.8 × (4 − Albumin) |
| Estimated Average Glucose (eAG) | = 28.7 × HbA1c − 46.7 |
| Estimated Glomerular Filtration Rate (eGFR) | from Creatinine, age, sex |
| FIB-4 Index | = (age × AST) ÷ (Platelets × √ALT) |
| Globulin | = Total Protein − Albumin |
| HOMA-IR Score | = Insulin × Blood Fasting Glucose ÷ 405 |
| Iron % Saturation | = Iron ÷ TIBC × 100 |
| LDL/HDL Ratio | = LDL ÷ HDL |
| Non-HDL Cholesterol | = Total Cholesterol − HDL |
| Plasma Osmolality | = 2×Na + Glucose÷18 + BUN÷2.8 |
| Remnant Cholesterol | = Total Cholesterol − HDL − LDL |
| Systemic Immune-Inflammation Index (SII) | = Platelets × Neutrophils ÷ Lymphocytes |
| Triglycerides/HDL Ratio | = Triglycerides ÷ HDL |

Costs 0 RON at all three providers — buy the inputs above and compute it.

## Extended

Beyond Core, Synevo covers 34 of 43 purchasable Extended biomarkers, Regina
Maria 28, and MedLife 34. Grouped by Whoop's five Specialized Panels; a biomarker
shared by more than one Specialized Panel repeats under each, marked `†`.

### Heart Health

| Whoop Biomarker | Synevo | Regina Maria | MedLife |
|---|---|---|---|
| Arachidonic Acid (AA) | Acizi grasi omega 3 si omega 6 † | — | Acizi grasi omega 3 si omega 6 † |
| Asymmetric Dimethylarginine (ADMA) | — | — | — |
| Cystatin C | Cistatina C | Cystatin C | Cistatina C |
| Docosahexaenoic Acid (DHA) | Acizi grasi omega 3 si omega 6 † | — | Acizi grasi omega 3 si omega 6 † |
| Docosapentaenoic Acid (DPA) | — | — | — |
| Eicosapentaenoic Acid (EPA) | Acizi grasi omega 3 si omega 6 † | — | Acizi grasi omega 3 si omega 6 † |
| HDL Large | — | — | — |
| LDL Medium | — | — | — |
| LDL Particle Number | — | — | — |
| LDL Peak Size | — | — | — |
| LDL Small | small dense LDL (sdLDL) | Profil LDL (LDL colesterol, sd-LDL colesterol, LDL oxidat, lipoproteina A) | Small dense LDL (sdLDL) |
| Linoleic Acid (LA) | Acizi grasi omega 3 si omega 6 † | — | Acizi grasi omega 3 si omega 6 † |
| Lipoprotein-Associated Phospholipase A₂ (Lp-PLA₂) | Lipoproteina asociata fosfolipazei A2 | — | Lipoproteina asociata fosfolipazei A2 |
| Myeloperoxidase (MPO) | — | — | — |
| Symmetric Dimethylarginine (SDMA) | — | — | — |
| Uric Acid (UA) | Acid uric seric † | Acid uric seric † | Acid uric † |

### Performance Health

| Whoop Biomarker | Synevo | Regina Maria | MedLife |
|---|---|---|---|
| Arachidonic Acid (AA) | Acizi grasi omega 3 si omega 6 † | — | Acizi grasi omega 3 si omega 6 † |
| Creatine Kinase (CK) | Creatinkinaza (CK) | Creatinkinaza (CK) | Creatinkinaza-CK |
| Docosahexaenoic Acid (DHA) | Acizi grasi omega 3 si omega 6 † | — | Acizi grasi omega 3 si omega 6 † |
| Docosapentaenoic Acid (DPA) | — | — | — |
| Eicosapentaenoic Acid (EPA) | Acizi grasi omega 3 si omega 6 † | — | Acizi grasi omega 3 si omega 6 † |
| Folate (Vitamin B9) | Folati serici † | Folati serici (acid folic) † | Folat seric † |
| Free T3 (FT3) | FT3 (triiodotironina libera) † | FT3 (triiodotironina libera) † | Free T3 † |
| Free T4 (FT4) | FT4 (Tiroxina libera) † | FT4 (tiroxina libera) † | Free T4 † |
| IGF-1 | IGF-I (somatomedin C) | IGF 1 (Factor 1 de inhibitie a cresterii) (IGF) | IGF-1 (somatomedin C) |
| Linoleic Acid (LA) | Acizi grasi omega 3 si omega 6 † | — | Acizi grasi omega 3 si omega 6 † |
| Magnesium | Magneziu seric † | Magneziu seric † | Magneziu † |
| Reticulocyte Count (RET) | Hemograma cu formula leucocitara, Hb,Ht,indici si reticulocite (Hemograma) | Hemoleucograma completa cu formula leucocitara, Hb, Ht, indici si reticulocite | Numaratoare de reticulocite |
| Vitamin B12 (cobalamin) | Vitamina B12 † | Vitamina B 12 † | Vitamina B12 † |

### Metabolic Health

| Whoop Biomarker | Synevo | Regina Maria | MedLife |
|---|---|---|---|
| 1,5-Anhydroglucitol (1,5-AG) | — | — | — |
| Adiponectin | Adiponectina | Adiponectina | Adiponectina |
| C-Peptide | Peptidul C | C-Peptid | Peptid C |
| Copper (serum) | Cupru in plasma | Cupru in ser/plasma | Cupru seric |
| Free T3 (FT3) | FT3 (triiodotironina libera) † | FT3 (triiodotironina libera) † | Free T3 † |
| Free T4 (FT4) | FT4 (Tiroxina libera) † | FT4 (tiroxina libera) † | Free T4 † |
| Fructosamine | Fructozamina | Fructozamina | Fructozamina |
| Gamma-Glutamyl Transferase (GGT) | GGT - Gama-glutamiltransferaza (Glutamiltranspeptidaza, Gama GT) | Gamaglutamiltranspeptidaza (GGT) | Gama GT |
| Iodine | Iod seric | — | Iod in sange |
| Leptin | Leptina † | Leptina † | Leptina † |
| Selenium | Seleniu | Seleniu in sange | Seleniu |
| Zinc (serum) | Zinc în plasmă † | Zinc in ser † | Zinc † |

### Women's Health

| Whoop Biomarker | Synevo | Regina Maria | MedLife |
|---|---|---|---|
| Anti-Müllerian Hormone (AMH) | Hormon anti-Müllerian (AMH) | Hormon antimullerian (AMH) | Hormon Anti-Mullerian |
| Folate (Vitamin B9) | Folati serici † | Folati serici (acid folic) † | Folat seric † |
| Free T3 (FT3) | FT3 (triiodotironina libera) † | FT3 (triiodotironina libera) † | Free T3 † |
| Free T4 (FT4) | FT4 (Tiroxina libera) † | FT4 (tiroxina libera) † | Free T4 † |
| Leptin | Leptina † | Leptina † | Leptina † |
| Magnesium | Magneziu seric † | Magneziu seric † | Magneziu † |
| Phosphorus | Fosfor seric | Fosfor seric | Fosfor seric |
| Progesterone | Progesteron | Progesteron | Progesteron |
| Prolactin | Prolactina † | Prolactina † | Prolactina † |
| Thyroid Peroxidase Antibodies (TPOAb) | Anti-TPO (Anti-tiroidperoxidaza) | Anticorpi anti TPO (ATPO) | Ac Anti-Tireoperoxidaza (ATPO) |
| Vitamin B12 (cobalamin) | Vitamina B12 † | Vitamina B 12 † | Vitamina B12 † |

### Men's Health

| Whoop Biomarker | Synevo | Regina Maria | MedLife |
|---|---|---|---|
| Dihydrotestosterone (DHT) | Dihidrotestosteron | Dihidrotestosteron | Dihidrotestosteron (DHT) |
| Leptin | Leptina † | Leptina † | Leptina † |
| Magnesium | Magneziu seric † | Magneziu seric † | Magneziu † |
| PSA, Free | Free PSA | Free PSA | Free PSA |
| PSA, Total | PSA (Antigen specific prostatic) | PSA (Antigen specific prostatic) | Ag. specific prostatic (PSA) |
| Prolactin | Prolactina † | Prolactina † | Prolactina † |
| Uric Acid (UA) | Acid uric seric † | Acid uric seric † | Acid uric † |
| Zinc (serum) | Zinc în plasmă † | Zinc in ser † | Zinc † |

† appears in more than one Specialized Panel, and repeats under each — you buy it
once. The shopping lists say what that saves.

**Not yet placed:** DHEA Sulfate is Extended but isn't part of any of Whoop's
five Specialized Panels, so it has no Block and no shopping-list line. Its
prices live here for want of anywhere better: Synevo 65 RON, Regina Maria 70
RON, MedLife 57 RON. See `CLAUDE.md` for why this is still open.

**Extended — Derived (9)**

| Derived Biomarker | Formula |
|---|---|
| Adiponectin/Leptin Ratio (LAR) | = Adiponectin ÷ Leptin |
| Arachidonic Acid/EPA Ratio (AA/EPA) | = AA ÷ EPA |
| Cystatin C eGFR | from Cystatin C, age, sex |
| LDL Pattern | threshold on LDL Peak Size |
| Omega-3 Total | = EPA + DPA + DHA |
| Omega-6 Total | = AA + LA |
| Omega-6/Omega-3 Ratio | = Omega-6 Total ÷ Omega-3 Total |
| PSA %, Free | = PSA Free ÷ PSA Total × 100 |
| Reticulocyte Index (RI) | = Reticulocyte % × (Hematocrit ÷ 45) |

Costs 0 RON at all three providers — buy the inputs above and compute it.

---

*Test names checked against each provider's catalogue — Synevo and Regina Maria
2026-08-04, MedLife 2026-08-05. Prices, and their own verification dates, are in
the shopping lists. Not medical advice — see [README](README.md) for the full
disclaimer and licence.*
